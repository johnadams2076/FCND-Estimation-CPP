...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
	...
	// Ensure predicted yaw is within the range -PI to PI 
	if (predictedYaw > F_PI)
	{
		predictedYaw -= 2.f * F_PI;
	}
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
	...
	//Rotate acceleration vector from body frame to inertial frame
	V3F intertialFrameAccel = attitude.Rotate_BtoI(accel);

	predictedState(3) += intertialFrameAccel.x * dt;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
	...
	MatrixXf tempMat  = gPrime * ekfCov;
